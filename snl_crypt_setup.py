...
def check_updates():
    try:
        ...
        if new_version != current_version:
            if messagebox.askyesno("Обновление", f"Доступна новая версия: {new_version}! Загрузить?"):
                ...
                progress_window.update()  # Обновляем окно перед загрузкой

                # Загружаем файл
                download_file(download_url, "update_file.exe")  # Имя загружаемого файла

                progress_window.destroy()  # Закрываем окно прогресса
                messagebox.showinfo("Загрузка завершена", "Обновление загружено! Запускаем...")
                subprocess.run(["update_file.exe"])  # Запускаем загруженный файл
            else:
                messagebox.showinfo("Обновление", "Вы остаетесь на текущей версии.")
        else:
